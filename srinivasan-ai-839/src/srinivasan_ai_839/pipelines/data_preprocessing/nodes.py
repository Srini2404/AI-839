...
def data_drift_quality_checks(df: pd.DataFrame) -> go.Figure:
    ...
    test_statuses = ['SUCCESS' if test['status'] == 'SUCCESS' else 'FAILURE' for test in results['tests']]
    ...
    fig.add_trace(go.Bar(
        x=test_names,
        y=[1] * len(test_names),  # Set y value to 1 for all tests (since we're focusing on status)
...
def data_drift_quality_checks_new(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Bar(
        x=categories,
        y=values,
        marker=dict(color=['green', 'orange', 'red']),  # Different colors for each category
        text=values,
        textposition='auto',
        hoverinfo='text'
    ))
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            tickvals=list(range(max(values) + 1))  # Adjust the y-axis ticks based on max value
        ),
        barmode='stack'
    )
